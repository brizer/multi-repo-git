...
const { findPlugins,registerPlugin } = require("../util/loadPlugins");
...
module.exports = v => {
  ...
  program.option("-l,--locals", "use locals dir but not config file");

  program.option("-e,--exclude <paths>", "exclude some path, connect with ,");
  //handler plugins first
  const userPlugins = findPlugins(process.cwd());
  if (userPlugins.size) {
    // 批量注册插件
  ...
  program
    ...
    .option(
      ...
      "update only when dependencies were found, otherwise not update"
    )
    .action((command, cmd) => {

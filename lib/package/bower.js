...
class Bower extends Package {
   ...
   */
  async setDependence({ name, branch }) {
    try {
      ...
      await nameList.map(async name => {
        ...
        this.pkg["resolutions"] = await this.setResolutions(name, `${branch}`);
      });
   ...
   */
  async setDependencies(key, value) {
    ...
    if (dependencies[key]) {
      ...
      console.log(
        color(`add ${this.filePath} 's dependencies ${key} to ${value} \n`)
          .white
      );
    }
  ...
  setResolutions(key, value) {
    ...
    if (resolutions[key]) {
      ...
      console.log(
        color(`add ${this.filePath} 's resolutions ${key} to ${value} \n`).white
      );
    }

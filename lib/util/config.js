...
class Config {
    ...
    async addp() {
        ...
        fs.writeJsonSync(this.findupFile(ConfigFileName),{projects:projects},{spaces:2})
    ...
    async rmp(){
        ...
        let newProjects = projects.filter(item=>{
            return item.path !==process.cwd()
        })

        fs.writeJsonSync(this.findupFile(ConfigFileName),{projects:newProjects},{spaces:2})

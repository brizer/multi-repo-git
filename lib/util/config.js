...
const { getConfigBackup } = require("./question");
...
class Config {
   ...
   */
  getConfig() {
    if (program.locals || this.program.locals) {
      return this.localsConfig();
    }
    if (this.config) {
  ...
  async addp(options = {}) {
    ...
    if (options.alias) {
      ...
      // Do not add again if there has one.
      projects = projects.filter((v) => v.alias !== options.alias);
  ...
  async ui() {
    ...
    const choosenProjectList = await getConfigBackup(
      backupList.map((v) => v.alias)
    );
    if (choosenProjectList.projects.length === 0) {
      console.log(color("please choose some project in backupList").red);
      process.exit(0);
    }

    // clean projects
    await this.clean();
    await Promise.each(choosenProjectList.projects, (v) => {
      return this.addp({ alias: v });
    }).catch((err) => {
      console.error(err);
    });
    console.log(color("config changed, use mrgx config ls to check").green);
  }
}
